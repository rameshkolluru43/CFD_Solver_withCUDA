...
**Note:** `CFD_solver_gpu` uses the **same CPU solver loop**. It does not run a separate GPU time-integration kernel in the main loop (CUDA is used for optional grid preprocessing and standalone kernels).
...
### CUDA inviscid flux path

| Item | Status |
|------|--------|
| `CFD_solver_gpu` 1st-order inviscid net flux | ✅ `Evaluate_Cell_Net_Flux_1O_CUDA()` |
...
| CUDA Roe (`Dissipation_Type=3`) | ⚠ Existing standalone Roe kernels exist, but the main loop falls back to CPU for this dispatcher |

Implementation files:

- `CUDA_KERNELS/Inviscid_Flux_Cuda_MainLoop.cu`: device kernel and kernel launcher.
- `CUDA_KERNELS/Inviscid_Flux_Cuda_MainLoop_Wrapper.cpp`: host bridge that flattens solver vectors, launches CUDA, and copies `Cells_Net_Flux` / `del_t` back.
- `src/Net_Flux.cpp`: calls `Evaluate_Cell_Net_Flux_1O_CUDA()` first when compiled with `USE_CUDA`; unsupported modes fall back to CPU.

### WENO path

| Item | Status |
|------|--------|
| `Is_WENO` → `Evaluate_Cell_Net_Flux_WENO()` | ✅ Quad cells (4 faces) |
...
|------|--------|
| `Viscous_Solver` loop | ✅ |
| `Evaluate_Viscous_Fluxes()` each iteration | ✅ `Grid_Refine_Functions.cpp` |
| Subtract `Cells_Viscous_Flux` in update | ✅ When `Is_Viscous_Wall` |
| RK stages recompute viscous flux | ✅ `Runge_Kutta_Method` |
| Wall skin friction output | ✅ Periodic + end of `Viscous_Solver` |
...
| **Viscous_Solver flow** | Runs | Calls `Evaluate_Cell_Net_Flux_*` then `Explicit_Method()` which **recomputes** inviscid flux (redundant) |
...
| **CUDA flux path** | 1st-order LLF/MOVERS/RICCA/MOVERS_NWSC | 2nd-order MUSCL, WENO, Roe dispatch, viscous flux, RK updates, and AMR split/merge still use CPU-side loop logic |
...
| **GPU RK / viscous / WENO** | 1st-order inviscid flux has a CUDA path; RK updates, viscous fluxes, WENO, and implicit remain CPU-side |
...
2. **GPU main loop** — Wire CUDA flux/RK/viscous into the solver iteration, or document CPU-only time marching for `CFD_solver_gpu`.
...
8. **Viscous_Solver** — Remove duplicate inviscid flux evaluation before `Explicit_Method`.
9. **Turbulence** — Route `Viscous_Solver_With_Turbulence` from `runSolver()` when enabled in JSON.
...
| [MESH_AND_GRID.md](MESH_AND_GRID.md) | Mesh formats and connectivity |
