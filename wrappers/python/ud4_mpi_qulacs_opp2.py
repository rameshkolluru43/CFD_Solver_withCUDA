...
size = comm.Get_size()
print(f"MPI rank {mpirank} out of {size} processes")
ATOL_DEFAULT = 1e-8
RTOL_DEFAULT = 1e-5
...
class Utility: 
    ...
    def generate_burgers_equation_matrix(self, n):
        size = 2**n
        dx = 1.0 / (size - 1)
...
class Qsolver(Utility,MaxIterationsReached): 
    ...
    def prepare_x(self, opt_weights: List[float], reps: int) -> np.ndarray:
        ...
        state = QuantumState(self.system_size, use_multi_cpu=True)
        
        # Var_circuit.update_quantum_state(state) 

        # optimization of the circuit

        Opt_type = "Heavy"  # Choose optimization type
        if Opt_type == "Light":
            swap_level = 1
            if state.get_device_name() == "multi-cpu":
                QuantumCircuitOptimizer().optimize_light(Var_circuit, swap_level)
            else:
                QuantumCircuitOptimizer().optimize_light(Var_circuit)
        elif Opt_type == "Heavy":
            max_block_size = 2
            swap_level = 2
            if state.get_device_name() == "multi-cpu":
                QuantumCircuitOptimizer().optimize(Var_circuit, max_block_size, swap_level)
            else:
                QuantumCircuitOptimizer().optimize(Var_circuit, max_block_size)
        Var_circuit.update_quantum_state(state)

        samples = state.sampling(self.shots, self.seed)
        counts = dict(Counter(samples))

        old_keys = [self.convert_bin(_, self.system_size) for _ in range(2**self.system_size)]
        new_keys = [old_keys[i] for i in range(2**self.system_size)]
        # new_count = {new_keys[i]: counts[i] for i in range(2**self.system_size)}
        new_count = {new_keys[i]: counts.get(i, 0) for i in range(2**self.system_size)}
        stv = np.array(list(new_count.values()))
        x = stv / self.shots
        print("x is prepared stmt inside prep_x fun")
        return x
    ...
    def cost(self, weights):
        ...
        b_state = QuantumState(self.tot_size, use_multi_cpu=True)
        ...
        if Opt_type == "Light":
            swap_level = 1
            if b_state.get_device_name() == "multi-cpu":
                QuantumCircuitOptimizer().optimize_light(cost_circuit, swap_level)
            else:
                QuantumCircuitOptimizer().optimize_light(cost_circuit)
        elif Opt_type == "Heavy":
            max_block_size = 2
            swap_level = 2
            if b_state.get_device_name() == "multi-cpu":
                QuantumCircuitOptimizer().optimize(cost_circuit, max_block_size, swap_level)
            else:
                QuantumCircuitOptimizer().optimize(cost_circuit, max_block_size)
        cost_circuit.update_quantum_state(b_state)
        samples = b_state.sampling(self.shots, self.seed)
        counts = dict(Counter(samples))


        old_keys = [self.convert_bin(_, self.tot_size) for _ in range(2**self.tot_size)]
        new_keys = [old_keys[i] for i in range(2**self.tot_size)]
        # new_count = {new_keys[i]: counts[i] for i in range(2**self.tot_size)}
        new_count = {new_keys[i]: counts.get(i, 0) for i in range(2**self.tot_size)}
        stv = np.array(list(new_count.values()))
