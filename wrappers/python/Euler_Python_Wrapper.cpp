...
PYBIND11_MODULE(CFD_Solver, m)
{
      ...
      m.def("testCase", [](int &Test_Case)
            { testCase(Test_Case); }, py::arg("Test_Case").noconvert(), "Function to set test case ID");

      m.def("Set_DelU", &Set_DelU, py::arg("dU"), "Sets the delU values for the cells");

      // Geometric functions
      m.def("ReadGrid", &Read_Grid, "Reads the input grid file");
      m.def("FormCells", &Form_Cells, "Constructs cells from the grid data - returns True on success, False on failure");
      ...
      m.def("Inviscid_Solver", &Inviscid_Solver,
            ...
            "Solves using the inviscid solver with specified error and solution files - returns True on success, False on failure.");

      m.def("Viscous_Solver", &Viscous_Solver,
            py::arg("error_file"), py::arg("solution_file"),
            "Solves using the viscous solver with specified error and solution files - returns True on success, False on failure.");
