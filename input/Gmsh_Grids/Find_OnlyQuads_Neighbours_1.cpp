...
set<int> readGeoFile(const string& geoFilePath) {
    set<int> cylinderEdges;
    ifstream file(geoFilePath);
    if (!file) {
        cerr << "Error: Unable to open .geo file." << endl;
        return cylinderEdges;
    }

    string line;
    while (getline(file, line)) {
        if (line.find("Physical Curve(\"CylinderWall\")") != string::npos) {  // ✅ Fixed escaping
            size_t start = line.find("{") + 1;
            size_t end = line.find("}");
            string ids = line.substr(start, end - start);
            stringstream ss(ids);
            int id;
            while (ss >> id) {
                cylinderEdges.insert(id);
...
// Function to read .vtk file and extract mesh data
void readVTKFile(const string& vtkFilePath, vector<vector<int>>& cells, vector<pair<double, double>>& points) {
    ...
    for (int i = 0; i < numCells; ++i) {
        ...
        if (cell->GetNumberOfPoints() == 4) { // Only quadrilateral cells
            vector<int> cellPoints;
            for (int j = 0; j < 4; ++j) {
                cellPoints.push_back(cell->GetPointId(j));
            }
