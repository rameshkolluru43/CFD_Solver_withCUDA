...
struct Config
{
    ...
    /* Open cavity (Case == cavity): rectangular cut in the floor, extruded in z */
    double cavity_x_start = 3.0;
    double cavity_x_end = 8.0;
    double cavity_depth = 1.0;
    double cavity_lip_width = 0.18; /* tanh blend at each lip (avoids one-cell cliffs) */
    double cavity_x_cluster = 1.7;  /* >1 packs x-nodes toward both lips */
    double post_rho = 1.0;
    double post_u = 0.0;
    double post_v = 0.0;
    double post_w = 0.0;
    ...
    std::string vtk_out = "solutions/2D_Euler_Solutions/Box_3D/sod_3d.vtk";
...
struct Mesh
{
    ...
    std::vector<int> active;  /* empty => all fluid; 0=solid (cavity blocks) */
...
inline bool cell_active(const Mesh &m, int c)
{
    return m.active.empty() || (c >= 0 && c < static_cast<int>(m.active.size()) && m.active[c] != 0);
}
