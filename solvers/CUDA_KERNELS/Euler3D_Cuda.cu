...
__device__ __forceinline__ void reconstruct_weno(
    ...
    const int *__restrict__ active, double UL[kNv], double UR[kNv])
{
    int ids[6];
#pragma unroll
    for (int s = 0; s < 6; ++s) {
        ids[s] = line_cell(cell, face, s - 2, nx, ny, nz);
        if (ids[s] < 0 || (active && !active[ids[s]]))
            return;
    }
    double sten[6][kNv];
#pragma unroll
                                ...
                                int ricca_sensor, double ricca_rh_thresh, int weno_hybrid)
{
    ...
    if (cell >= n_cells || (active && !active[cell]))
        return;
...
#pragma unroll
            ...
            if (order == 2)
                reconstruct_weno(U, cell, face, nx, ny, nz, fnx, fny, fnz,
                                 weno_char, weno_z, weno_hybrid, active, UL, UR);
        } else {
...
__global__ void viscous_grad_kernel(const double *__restrict__ U, const int *__restrict__ neigh,
                                    ...
                                    int n_cells, const int *__restrict__ active)
{
    const int cell = blockIdx.x * blockDim.x + threadIdx.x;
    if (cell >= n_cells || (active && !active[cell]))
        return;
...
void allocate(DeviceBundle &dev, int n_cells)
{
    ...
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&dev.d_mu_sgs),
                          sizeof(double) * n_cells));
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void **>(&dev.d_active),
                          sizeof(int) * n_cells));
}
                 ...
                 double dx, double dy, double dz)
{
    ...
    if (active.size() == cells)
        CUDA_CHECK(cudaMemcpy(dev.d_active, active.data(), sizeof(int) * cells,
                              cudaMemcpyHostToDevice));
    else {
        std::vector<int> ones(cells, 1);
        CUDA_CHECK(cudaMemcpy(dev.d_active, ones.data(), sizeof(int) * cells,
                              cudaMemcpyHostToDevice));
    }
    dev.dx = dx;
    dev.dy = dy;
                     ...
                     int ricca_sensor, double ricca_rh_thresh, int weno_hybrid)
{
    ...
    net_flux_kernel<<<blocks, kBlockSize>>>(
        ...
        dev.d_freestream, dev.d_postshock, dev.d_active, dev.n_cells, dev.nx, dev.ny, dev.nz,
                         ...
                         double A_plus, double Uinf)
{
    ...
    viscous_grad_kernel<<<blocks, kBlockSize>>>(dev.d_U, dev.d_neigh, dev.d_bc, dev.d_xc,
                                                ...
                                                dev.dy, dev.dz, dev.n_cells, dev.d_active);
    CUDA_CHECK(cudaGetLastError());
