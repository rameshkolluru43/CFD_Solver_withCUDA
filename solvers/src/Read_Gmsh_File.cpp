...
bool Read_VTK_Grid(const string &GridFileName)
{
    try
    {
        ifstream file = CFD_OpenInputFileOrThrow(GridFileName, "Read_VTK_Grid");
...
void Identify_Cells(V_D &Points, vector<Cell> &Cells, bool Is_2D_Flow, int &No_Physical_Cells)
{
    ...
    else
    {
        ...
        throw runtime_error("Identify_Cells: physical cell count mismatch");
    }
    ...
    for (size_t i = 0; i < Cells.size(); i++)
    {
        ...
        if (Is_2D_Flow)
        {
            ...
            else
            {
                ...
                throw runtime_error("Identify_Cells: unsupported cell type");
            }
            Cells[i].Dimension = 2;
...
void Read_GmshMESH_Grid(const string &GridFileName)
{
    ...
    ifstream file = CFD_OpenInputFileOrThrow(GridFileName, "Read_GmshMESH_Grid");
    Is_2D_Flow = true;
    string line;
    vector<double> Points;
