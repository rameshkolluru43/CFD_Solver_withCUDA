		...
		do
		{
			...
			if (use_resident_gpu)
			{
				...
				if (!Resident_GPU_Explicit_Step(Min_dt, err4))
				{
					...
					/* Do NOT download device state here — it may already be non-physical.
					   Host retains the last periodic snapshot (or IC). */
				...
				else if (iterations > 0 && ((iterations + 1) % 1000 == 0))
				{
					/* Keep a recent good host snapshot for safe fallback. */
