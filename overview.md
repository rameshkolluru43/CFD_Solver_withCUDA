...
| Target | Role |
|--------|------|
| `CFD_solver` | CPU build (VTK required) |
| `CFD_solver_gpu` | Same driver + CUDA kernels where enabled |
...
./build-cuda/CFD_solver_gpu json_Files/<run>.json
```
...
- Grid: `Grid_Files/Half_Cylinder/Half_Cylinder_481_161.txt` (480×160 cells)
- Inviscid reference: WENO + RICCA, **Pmax ≈ 48.3**, **Mmax ≈ 6**  
  `results/ordered_sweep/P3_RICCA_WENO/`
- Viscous: isothermal no-slip wall, Re = 1e5, Tw = 2  
  Case JSON: `json_Files/Half_Cylinder_481_161_M6_viscous.json`  
  Smoke after corner fix: `results/halfcyl_corner_fix_smoke500/` (**Pmax ≈ 48.9**, **Mmax ≈ 6.05** at 500 iters from P3)
- Plots: `plots/half_cylinder_inviscid_viscous/`  
  Regenerate: `python3 scripts/plot_halfcyl_inviscid_viscous.py`
